...
def search_transcript(vid_id, words):
    ...
    for line in transcript:
        # "start" is a string respresing the start time. this makes start an int.
        line["start"] = int(line["start"])
        found_words = []
        found_words = list(
            filter(lambda word: line["text"].upper().find(word) != -1, words))
        if(found_words):
            # if the word is found in the line, catalogue the line
            found_lines.append({"found_words": found_words, "line": line})
            word_count = word_count + 1
            print(word_count)

    # found_lines contains an entry for every line a word is found on.
    return [found_lines, word_count]
...
def newMain(request):
    ...
    for index, vid_id in enumerate(video_ids):
        ...
        vid = {"vid_id": vid_id, "transcript": True,
               "title": video_titles[index], "found_lines": found_lines, "word_count": word_count}
